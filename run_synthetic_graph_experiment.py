...
from run_partition_agg_experiment import load_validation_data, eval_all_fraud_detectors, load_test_data
...
def generate_synthetic_datasets(eps, deg_cutoff_rate, iters=10, non_private=False, statistics_only=False, 
                                    save_data=False, run_parallel=False, split='val'):
    if split == 'val':
    ...
    if run_parallel:
        ...
        for i in range(iters):
            data = run_single_iter_generate_synthetic_datasets(d, eps, deg_cutoff_rate, i, statistics_only, non_private, save_data, split=split)
            out['data'][i] = data
    
    return out

def run_single_iter_generate_synthetic_datasets(d, eps, deg_cutoff_rate, i, statistics_only=False, non_private=False, save_data=False, split='val'):
    ...
    for name, data in d.items():
        ...
        if not statistics_only: # evaluate AUCs
            ...
            for algo, graphs in zip(['sbm_dp', 'attr_graph_simp', 'attr_graph', 'topmfilter'], [graphs_sbm, graphs_agm_simp, graphs_agm, graphs_topm]):
                ...
                aucs_file = f'synthetic_graphs/aucs_{split}_{int(100*deg_cutoff_rate)}_{int(eps)}_{i}.csv'

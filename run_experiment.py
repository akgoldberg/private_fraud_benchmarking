...
def load_validation_data():
    dataset_names = ['yelp', 'peer_review', 'elliptic']
    datasets = [load_data.load_yelp_data(train=True), load_data.load_peer_review_data(train=True), load_data.load_elliptic_data(train=True)]
...
def load_test_data():
    ...
    datasets = [load_data.load_amazon_data(), load_data.load_yelp_data(train=False), 
                    load_data.load_FDcomp_data(), load_data.load_peer_review_data(train=False), load_data.load_elliptic_data(train=False)]
...
def run_evaluation_non_private(dataset_names, datasets, niters=10):
    out = {}
...
def grid_search_svd_threshold():
    dataset_names, datasets = load_validation_data()

    res = {}

    for name, (A, labels, _) in zip(dataset_names, datasets):
        res[name] = {}
...
def grid_search_partition_aggregate_params():
    ...
    for k in [5, 10, 20, 50, 100, 200]:
        for sub_rate in [0.01, 0.1, 0.25, 0.5, 0.75, 1.0]:
            print('=======Running with k:', k, 'sub_rate=========', sub_rate)
            if sub_rate < 1./k:
                continue
            res = run_evaluation_subsample_aggregate(dataset_names, datasets, k, sub_rate)
            out[k, sub_rate] = res
...
def run_evaluation_subsample_aggregate(dataset_names, datasets, k, sub_rate, iters=10):
    out = {}
...
################################################################
######## Run Experiments on Estimated SBMs #####################
################################################################
...
def estimate_sbms():
    ...
    for name, data in zip(dataset_names, datasets):
        ...
        params = sbm.est_sbm_params(A, labels)
        print(f'{name} n0: {params[0]}, n1: {params[1]}, p00: {params[2]}, p11: {params[3]}, p01: {params[4]}')
...
if __name__ == '__main__':
    ...
    dataset_names, datasets = load_test_data()
    res = run_evaluation_non_private(dataset_names, datasets)

...
VAL_DATASETS = ['yelp', 'peer_review', 'elliptic']
...
def generate_nonprivate_leaderboard(split='val', print_latex=False):
    ...
    return generate_leaderboard(df, VAL_DATASETS if split=='val' else TEST_DATASETS, algos, print_latex=print_latex)
        ...
        for k in laplace_aggs['k'].unique():
            for iter in laplace_aggs['iter'].unique():
                priv_leaderboard = generate_leaderboard(laplace_aggs[(laplace_aggs['sub_rate'] == sub_rate) 
                                                                    & (laplace_aggs['k'] == k) & (laplace_aggs['iter'] == iter)], VAL_DATASETS, algos)
...
def plot_pda_validation_results(stat='bias', method='laplace', col='k', x='sub_rate', eps=1.0, dataset = 'yelp'):
    df = analyze_pda_validation_results_leaderboard(eps=10*eps)
...
if __name__ == '__main__':
    ...
    # df = pd.concat([analyze_pda_validation_results_leaderboard(eps=10.*eps) for eps in [0.1, 0.25, 0.5, 1.0, 1.5, 2.0, 2.5]])
    ...
    # plot_pda_validation_results_leaderboard(10.0)
...
def estimate_sbm_params_dp(A, labels, eps, deg_cutoff, fraud_private = True, noise_type = 'cauchy', delta=1e-8):
    ...
    if fraud_private:
        ...
        if noise_type == 'laplace':
            n00 += add_laplace_noise(eps0, delta, sens, A, labels, deg_cutoff, truncate_fruad=fraud_private)
            n11 += add_laplace_noise(eps1, delta, sens, A, labels, deg_cutoff, truncate_fruad=fraud_private)
            n01 += add_laplace_noise(eps2, delta, sens, A, labels, deg_cutoff, truncate_fruad=fraud_private)
        else: 
            n00 += add_cauchy_noise(eps0, sens, A, labels, deg_cutoff, truncate_fruad=fraud_private)
            n11 += add_cauchy_noise(eps1, sens, A, labels, deg_cutoff, truncate_fruad=fraud_private)
            n01 += add_cauchy_noise(eps2, sens, A, labels, deg_cutoff, truncate_fruad=fraud_private)
    else:
        # weight eps by size of denominator
        ...
        if noise_type == 'laplace':
            n00 += add_laplace_noise(eps0, delta, sens, A, labels, deg_cutoff, truncate_fruad=fraud_private)
            n01 += add_laplace_noise(eps1, delta, sens, A, labels, deg_cutoff, truncate_fruad=fraud_private)
        else:   
            n00 += add_cauchy_noise(eps0, sens, A, labels, deg_cutoff, truncate_fruad=fraud_private)
            n01 += add_cauchy_noise(eps1, sens, A, labels, deg_cutoff, truncate_fruad=fraud_private)
